...
export class ConfigLoader {
  ...
  async load(configPath: string = './migrate.json'): Promise<MigrateConfig> {
    ...
    const config: MigrateConfig = {
      ...
      transactionMode:
        (process.env['TRANSACTION_MODE'] as 'auto' | 'manual') ||
      ...
      logging: {
        level:
          ...
          'info',
        format:
          (process.env['LOG_FORMAT'] as 'json' | 'text') || fileConfig.logging?.format || 'text',
    ...
    if (!config.databaseUrl) {
      throw new Error(
        'DATABASE_URL not configured. Set DATABASE_URL environment variable or add to migrate.json',
      );
    }
