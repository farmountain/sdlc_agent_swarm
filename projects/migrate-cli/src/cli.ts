...
program.name('migrate').description('Database migration management CLI tool').version('1.0.0');
    ...
    try {
      ...
      if (results.length === 0) {
        ...
        results.forEach((result) => {
          if (result.success) {
            console.log(
              chalk.green(`✓ Rolled back ${result.migration.name} (${result.durationMs}ms)`),
            );
          } else {
            console.log(chalk.red(`✗ ${result.migration.name} - ${result.error?.message}`));
          }
