...
export class SQLiteAdapter implements IDatabaseAdapter {
  ...
  async acquireLock(_key: number, _timeoutSeconds: number): Promise<void> {
    if (!this.db) {
      throw new Error('Not connected to database');
    }
    ...
    try {
      ...
      if ((error as Error).message.includes('SQLITE_BUSY')) {
        throw new Error(
          'Database is locked by another process (SQLite does not support advisory lock timeouts)',
        );
      }
  ...
  async releaseLock(_key: number): Promise<void> {
